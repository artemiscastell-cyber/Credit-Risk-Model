...
* **AUC Score: 0.7982**
* **數值意義**：
    * AUC 代表模型區分好壞客戶的能力。**0.7988** 的分數意味著：如果我們隨機挑選一位「違約客戶」和一位「正常客戶」，模型有 **79.8%** 的機率會正確給予違約客戶較高的風險分數。
    * 在實務中，AUC > 0.7 通常被視為具備上線水準，本模型接近 0.8。

### 2. 穩健性檢定 (Robustness Check)
為了避免模型只是「運氣好」切到簡單的測試集，我們進行了 5-Fold Cross Validation：
* **平均 AUC**：**0.7502**
* **標準差**：0.0580
...
模型不僅能預測誰會違約，更能解釋**「為什麼」**。透過將邏輯斯迴歸的係數 ($\beta$) 轉換為 **勝算比 (Odds Ratio, $OR = e^\beta$)**，我們可以量化各特徵對風險的倍數影響。
